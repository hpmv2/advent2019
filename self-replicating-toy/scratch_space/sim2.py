...
def quine_payload():
    ...
    escaped_payload = quine_escape(payload[::-1])
    return quine_escape(escaped_payload[::-1])
...
open('example_solution', 'wb').write(quine_code)

x = process('./chal')
x.sendline(str(len(quine_code)))
x.send(quine_code)
print hexdump(x.recvall())
